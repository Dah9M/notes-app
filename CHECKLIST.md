...
- [ ] **Ручная проверка UI в браузере** — в процессе. Найден и исправлен баг: `AuthInterceptor` перехватывал CORS preflight (`OPTIONS`) и требовал `X-User-Id`, которого preflight-запрос не несёт → браузер получал `NetworkError` при любом запросе с телом (create/update). Исправлено: `OPTIONS` пропускается без проверки. Проверено curl'ом с заголовками `Origin`/`Access-Control-Request-*`, как их шлёт браузер — preflight 200, реальный POST 201. Нужно ещё раз пройти сценарий в реальном браузере: регистрация → логин → создать/отредактировать/удалить заметку → фильтр по тегу → логаут
